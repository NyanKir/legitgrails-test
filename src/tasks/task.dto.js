...
class TaskDto {
    constructor(task) {
        ...
        if (this.status === 'update-needed') {
            ...
            this.photos_to_resubmit = task?.photos_to_resubmit ? task.photos_to_resubmit.split(", ") : [];
        }

        if (this.status === 'scheduled' || PROCESSING_STATUSES.includes(this.status)) {
            ...
            this.expires_readable = seconds < 0 ? "0 days, 0 hours, 0 minutes" : expires_readable
        }
